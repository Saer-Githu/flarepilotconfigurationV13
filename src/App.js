...
function compute(v, isGasOn) {
  ...
  if (phi >= 0.5 && phi <= 2.8) {
    prelim_tF = 1980 * (1 - 0.3 * Math.abs(1 - phi) ** 1.2);
    prelim_tF = Math.max(v.amb_temp, Math.min(1980, prelim_tF));
  }
...
function Diagram({ inp, out, isIgnited, isGasOn }) {
  const W = 450,
    H = 800,
    cx = W / 2,
    cyBase = 700,
  ...
  return (
    ...
    >
      ...
      <line
        ...
        y1={cyBase + 30}
        x2={isGasOn ? cx + 14 : cx}
        y2={isGasOn ? cyBase + 30 : cyBase + 16}
        stroke={isGasOn ? "#10b981" : "#ef4444"}
        strokeWidth={4}
        strokeLinecap="round"
      />
      ...
      <line
        ...
        x2={cx - cID / 2 - 10}
        y2={airY}
        stroke={isGasOn ? "#38bdf8" : "#334155"}
        strokeWidth={2}
        markerEnd={isGasOn ? "url(#arrow)" : ""}
      />
      <line
        x1={cx + cOD / 2 + 30}
        y1={airY}
        ...
        <>
          ...
          {Array.from({ length: Math.min(6, inp.rows) }, (_, i) => {
            ...
            return (
              <g key={`flame-${i}`}>
                <path
                  d={`M ${cx - headOD / 2} ${py + pD} Q ${
                    cx - headOD / 2 - 15
                  } ${py + 5} ${cx - headOD / 2 - 35} ${py - 15} Q ${
                    cx - headOD / 2 - 10
                  } ${py - 5} ${cx - headOD / 2} ${py - pD} Z`}
                  fill="#f97316"
                  opacity="0.85"
                  filter="url(#glow)"
                />
                <path
                  d={`M ${cx - headOD / 2} ${py + pD / 2} Q ${
                    cx - headOD / 2 - 10
                  } ${py + 2} ${cx - headOD / 2 - 20} ${py - 10} Q ${
                    cx - headOD / 2 - 5
                  } ${py - 2} ${cx - headOD / 2} ${py - pD / 2} Z`}
                  fill="#fde047"
                  opacity="0.9"
                />
                <path
      ...
      <text
        x={cx - 25}
        y={cyBase + 35}
        fill={isGasOn ? "#10b981" : "#ef4444"}
        fontSize={10}
        fontFamily="monospace"
        textAnchor="end"
      >
        {isGasOn ? "VALVE OPEN" : "VALVE CLOSED"}
      </text>
      <text
        x={cx - cOD / 2 - 35}
        y={airY - 10}
        fill={isGasOn ? "#38bdf8" : "#334155"}
        fontSize={10}
        fontFamily="monospace"
        textAnchor="end"
      >
      ...
      <text
        ...
        fontFamily="monospace"
      >
        OD: {(inp.body_id + 6.6).toFixed(1)}mm
      </text>
      <text
        x={cx}
        y={midBreak}
...
function doExport(inp, out, isIgnited, isGasOn) {
  ...
  const data = [
    ...
    [
      "Ignition State",
      isIgnited && out.comb_ok ? "ACTIVE" : !isIgnited ? "STANDBY" : "FAILED",
      "—",
      out.fail_reason,
    ],
  ];
...
export default function FlarePilotApp() {
  ...
  const out = useMemo(() => compute(inp, isGasOn), [inp, isGasOn]);
  ...
  // Determine Main Status UI Sequence
  let statusText = "SYSTEM STANDBY";
  let statusColor = "#38bdf8";
  let statusBg = "#0f172a";

  if (!isIgnited && !isGasOn) {
    statusText = "STANDBY (VALVE CLOSED)";
  } else if (!isIgnited && isGasOn) {
    statusText = "READY (GAS FLOWING UNIGNITED)";
    statusColor = "#f59e0b";
    statusBg = "#78350f";
  } else if (isIgnited && !isGasOn) {
    statusText = "BLOWOUT / SPARKING (NO GAS)";
    statusColor = "#fca5a5";
    statusBg = "#7f1d1d";
  } else if (isIgnited && isGasOn) {
    if (out.comb_ok) {
      statusText = "STABLE COMBUSTION";
      statusColor = "#34d399";
      statusBg = "#064e3b";
    } else {
      ...
      >
        ...
        <div style={{ display: "flex", gap: 15 }}>
          ...
          <button
            ...
            style={{
              background: isGasOn ? "#10b981" : "#475569",
              color: "white",
              border: "none",
              padding: "10px 20px",
              borderRadius: 6,
              ...
              fontFamily: "'Rajdhani',sans-serif",
              fontSize: 16,
              border: isGasOn ? "2px solid #059669" : "2px solid transparent",
            }}
          >
            {isGasOn ? "CLOSE GAS VALVE" : "OPEN GAS VALVE"}
          </button>
          <button
            onClick={() => setIsIgnited(!isIgnited)}
            style={{
              background: isIgnited ? "#b45309" : "#ef4444",
              color: "white",
              border: "none",
              padding: "10px 20px",
              borderRadius: 6,
          ...
          >
            {isIgnited ? "SHUTDOWN SPARK" : "IGNITE SYSTEM"}
          </button>
          <button
            onClick={() => doExport(inp, out, isIgnited, isGasOn)}
            style={{
              background: "#3b82f6",
              color: "white",
              border: "none",
      ...
      >
        ...
        >
          ...
          <Slider
            ...
            step={1}
            value={inp.top_od}
            onChange={(v) => update("top_od", v)}
            color="#10b981"
          />
          <Slider
            label="Top Head Length"
            unit="mm"
            min={50}
            max={500}
            step={10}
